package com.zbPro.seed.util;

/**
 * Created by Administrator on 2016/6/7.
 * ClassName ：com.zbPro.seed.util
 * 作用：
 */
public class Constant {
    public static final String PATH = "http://192.168.23.1:8080/";
    public static final String LOGIN = "login.do";
    public static final String REGISTER = "register.do";
    public static final String SEED = "seed.do";
    public static final String ROGUE = "rogue.do";
    public static final String CASTRATION = "castration.do";
    public static final String GAIN = "gain.do";
    public static final String FARMER = "farmer.do";
    public static final String LOGINFARMER = "farmer.do";
    public static final String TECHNICIAN = "technician.do";
    public static final String EVERYDAY = "everyday.do";

    //管理员访问的页面
    public static final String IMPORTANT = "important.do";
    public static final String ADMIN_TODAY = "adminToday1.do";

    public static final String ADMIN_SEED = "adminseed.do";

    public static final String ADMIN_CASTRATION="/adminCastration.do";
    public static final String ADMIN_ROGUE  = "/adminRogue.do";
    public static final String ADMIN_GAIN = "/adminGain.do";
    public static final String ADMINIMPORTANTTITLE = "/adminimportanttitle.do";
    public static final String ADMINIMPORTANT = "/adminimportant.do";

    public static final String FANGUI = "/fangui.do";
}
